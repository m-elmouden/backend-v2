...
@Service
public class ClasseComptableChercheurServiceImpl extends AbstractServiceImpl<ClasseComptable> implements ClasseComptableChercheurService {
...
private ClasseComptableDao classeComptableDao;
    ...
    private ArchivableService<ClasseComptable> archivableService;
        @Autowired
        private SousClasseComptableChercheurService sousClasseComptableService ;
...
public List<ClasseComptable> findAll(){
    List<ClasseComptable> resultat= new ArrayList();
...
return  classeComptableDao.save(classeComptable);
    ...
    private void prepareSave(ClasseComptable classeComptable){
        classeComptable.setDateCreation(new Date());
            ...
            classeComptable.setUsername(currentUser.getUsername());
            }
...
classeComptables.forEach(e->classeComptableDao.save(e));
        ...
        public List<ClasseComptable> findAllNonArchive(){
        String query = "SELECT o FROM ClasseComptable o  WHERE o.archive != true AND o.visible = true";
        return entityManager.createQuery(query).getResultList();
        }

        public List<ClasseComptable> findAllByOwner(){
        List<ClasseComptable> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM ClasseComptable o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
