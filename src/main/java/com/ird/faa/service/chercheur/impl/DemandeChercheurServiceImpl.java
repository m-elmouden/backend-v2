...
import com.ird.faa.service.chercheur.facade.DemandeChercheurService;
        ...
        import com.ird.faa.service.chercheur.facade.EtatDemandeChercheurService;
...
@Service
public class DemandeChercheurServiceImpl extends AbstractServiceImpl<Demande> implements DemandeChercheurService {
...
return  demandeDao.save(demande);
    ...
    private void prepareSave(Demande demande){
        demande.setDateCreation(new Date());
            ...
            demande.setUsername(currentUser.getUsername());
            }
...
demandes.forEach(e->demandeDao.save(e));
        ...
        public List<Demande> findAllNonArchive(){
        String query = "SELECT o FROM Demande o  WHERE o.archive != true AND o.visible = true";
            query+= " ORDER BY o.dateDemande";
        return entityManager.createQuery(query).getResultList();
        }

        public List<Demande> findAllByOwner(){
        List<Demande> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM Demande o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
            query+= " ORDER BY o.dateDemande";
