...
import com.ird.faa.service.chercheur.facade.DeclarationIrChercheurService;
        ...
        import com.ird.faa.service.chercheur.facade.PaiementDeclarationIrChercheurService;
...
@Service
public class DeclarationIrChercheurServiceImpl extends AbstractServiceImpl<DeclarationIr> implements DeclarationIrChercheurService {
...
private DeclarationIrDao declarationIrDao;
    ...
    private ArchivableService<DeclarationIr> archivableService;
        ...
        @Autowired
        private PaiementDeclarationIrChercheurService paiementDeclarationIrService ;
        @Autowired
        private EtatDeclarationIrChercheurService etatDeclarationIrService ;
        @Autowired
        private DeclarationIrEmployeChercheurService declarationIrEmployeService ;
        @Autowired
        private PrelevementSocialEmployeChercheurService prelevementSocialEmployeService ;
...
public List<DeclarationIr> findAll(){
    List<DeclarationIr> resultat= new ArrayList();
...
return  declarationIrDao.save(declarationIr);
    ...
    private void prepareSave(DeclarationIr declarationIr){
        declarationIr.setDateCreation(new Date());
            ...
            declarationIr.setUsername(currentUser.getUsername());
            }
...
declarationIrs.forEach(e->declarationIrDao.save(e));
        ...
        public List<DeclarationIr> findAllNonArchive(){
        String query = "SELECT o FROM DeclarationIr o  WHERE o.archive != true AND o.visible = true";
        return entityManager.createQuery(query).getResultList();
        }

        public List<DeclarationIr> findAllByOwner(){
        List<DeclarationIr> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM DeclarationIr o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
