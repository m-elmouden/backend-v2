...
@Service
public class DeclarationCnssDetailChercheurServiceImpl extends AbstractServiceImpl<DeclarationCnssDetail> implements DeclarationCnssDetailChercheurService {
...
public List<DeclarationCnssDetail> findAll(){
    List<DeclarationCnssDetail> resultat= new ArrayList();
...
return  declarationCnssDetailDao.save(declarationCnssDetail);
    ...
    private void prepareSave(DeclarationCnssDetail declarationCnssDetail){
        declarationCnssDetail.setDateCreation(new Date());
            ...
            declarationCnssDetail.setUsername(currentUser.getUsername());
            }
...
declarationCnssDetails.forEach(e->declarationCnssDetailDao.save(e));
        ...
        public List<DeclarationCnssDetail> findAllNonArchive(){
        String query = "SELECT o FROM DeclarationCnssDetail o  WHERE o.archive != true AND o.visible = true";
        return entityManager.createQuery(query).getResultList();
        }

        public List<DeclarationCnssDetail> findAllByOwner(){
        List<DeclarationCnssDetail> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM DeclarationCnssDetail o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
